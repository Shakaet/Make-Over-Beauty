...
const LinkItem = ({ href = '#', children }) => (
  <li>
    <a href={href} className="block py-1.5 text-sm text-black/70 hover:text-black transition-colors">
      {children}
    </a>
  </li>
)
...
const SocialIcon = ({ label, path }) => (
  <a
    aria-label={label}
    href="#"
    className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-black/20 text-black/70 hover:text-black hover:border-black transition-colors"
  >
...
const Footer = () => {
  ...
  return (
    <footer className="bg-[#f3ede6] text-[#0a0a0a]">
      {/* Top grid */}
      <div className="mx-auto max-w-7xl px-6 sm:px-8 md:px-10 lg:px-12 xl:px-16 py-12 pt-40">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-10">
          {/* Brand and blurb */}
          <div className="lg:col-span-2">
            <div className="text-5xl sm:text-6xl font-serif italic tracking-wide">LILAC</div>
            <p className="mt-6 text-sm leading-7 text-black/70 max-w-md">
              Sed viverra tellus in hac habitasse platea dictumst vestibulum. Mauris augue neque gravida in.
          ...
          {/* <div className="sm:col-span-2 md:col-span-2 lg:col-span-1">
            ...
            <div className="mt-4 flex w-full max-w-sm">
              <input
                ...
                className="flex-1 rounded-l-md border border-black/20 bg-white/60 px-3 py-2 text-sm outline-none placeholder:text-black/50 focus:ring-2 focus:ring-black/20"
              />
              <button
                aria-label="Subscribe"
                className="rounded-r-md border border-l-0 border-black/20 bg-[#e5d9c9] px-4 hover:bg-[#decfb9] transition-colors"
              >
        ...
        {/* Bottom bar */}
        <div className="mt-10 border-t border-black/10 pt-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          {/* Payments */}
          ...
          {/* Copyright */}
          <div className="text-sm text-black/70">© {year} lilac, Wedesign Tech.</div>

          {/* Legal */}
          <div className="flex items-center gap-4 text-sm">
            <a href="#" className="text-black/70 hover:text-black">Privacy Policy</a>
            <span className="text-black/30">|</span>
            <a href="#" className="text-black/70 hover:text-black">Terms</a>
          </div>
        </div>
      </div>
    </footer>
