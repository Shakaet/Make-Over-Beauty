          ...
          <div className={`${scrolled ? 'fixed top-0 inset-x-0 opacity-40 bg-gray-100 ' : 'absolute inset-x-0 top-0 bg-[#ffdcdc] mt-10 border-b border-black/10 shadow-sm'} z-30 transition-colors`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                        ...
                        <div className="group inline-flex items-center">
                          ...
                          <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity duration-150 absolute left-0 top-full mt-1 w-56 bg-[#ffdcdc] text-black border border-black/10 shadow-xl z-40">
                            <ul className="py-2">
                              ...
                              <li>
                                <Link href="/pages/faq" className="block px-4 py-2 hover:bg-black/5">Faq</Link>
                              </li>
                              <li>
                                <Link href="/pages/contact" className="block px-4 py-2 hover:bg-black/5">Contact Us</Link>
                              </li>
                              <li>
                                <Link href="/pages/404" className="block px-4 py-2 hover:bg-black/5">404</Link>
                              </li>
                            </ul>
                          </div>
                        </div>
                      ) : (
                        <Link href={`${item === 'Home' ? '/' : `/${item.toLocaleLowerCase()}`}`} className="inline-flex items-center gap-1 hover:text-black/80 transition-colors">
                          <span className="tracking-[0.25em] uppercase">{item}</span>
                        </Link>
                      )}
                    </li>
              ...
              <div className={`md:hidden ${mobileOpen ? 'block' : 'hidden'} pb-4`}>
                <ul className="text-sm text-black space-y-1 pt-2 border-t border-black/10">
                  {['Home','About','Shop','Blog','Gallery'].map((item) => (
                    <li key={item}>
                      <Link href={`${item === 'Home' ? '/' : `/${item.toLocaleLowerCase()}`}`} className="block px-1 py-2" onClick={() => setMobileOpen(false)}>
                        <span className="tracking-[0.25em] uppercase">{item}</span>
                      </Link>
                    </li>
                  ))}
                  <li className="pt-2 mt-1 border-t border-black/10">
                    <span className="block px-1 py-2 tracking-[0.25em] uppercase">Pages</span>
                    <ul className="pl-3 space-y-1">
                      <li>
                        <Link href="/pages/history" className="block px-1 py-2" onClick={() => setMobileOpen(false)}>Our History</Link>
                      </li>
                      <li>
                        <Link href="/pages/faq" className="block px-1 py-2" onClick={() => setMobileOpen(false)}>Faq</Link>
                      </li>
                      <li>
                        <Link href="/pages/contact" className="block px-1 py-2" onClick={() => setMobileOpen(false)}>Contact Us</Link>
                      </li>
                      <li>
                        <Link href="/pages/404" className="block px-1 py-2" onClick={() => setMobileOpen(false)}>404</Link>
                      </li>
                    </ul>
                  </li>
                </ul>
