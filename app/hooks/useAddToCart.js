...
export default function useAddToCart() {
  ...
  const loadCart = useCallback(() => {
    try {
      ...
      const validCart = stored.filter(
        (item) =>
          item.addedAt && now - item.addedAt < CART_EXPIRY_MINUTES * 60 * 1000,
      );
      setCart(validCart);
  ...
  const getProductPrice = (product) => {
    // Your product structure: highprice has the value, lowprice is null
    // Calculate final price with discount
    const basePrice =
      product.highprice || product.lowprice || product.price || 0;
  ...
  const addToCart = useCallback(
    (product, quantity = 1, onSuccess) => {
      try {
        const { _id, name, imagePrimary, stock, discount } = product;

        // Get price with fallback - use highprice instead of lowprice
        ...
        if (existingIndex !== -1) {
          ...
          existingCart.push({
            ...
            name,
            image: imagePrimary,
            price: price, // Now using correct price
            quantity,
            stock,
