...
export const createProduct = async (productData, images) => {
    try {
        ...
        // Append images
        if (images.imagePrimary) formData.append('imagePrimary', images.imagePrimary);
        if (images.imageSecondary) formData.append('imageSecondary', images.imageSecondary);
...
export const updateProduct = async (id, productData, images = {}) => {
    try {
        ...
        const res = await api.patch(`/api/products/update-product/${id}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
